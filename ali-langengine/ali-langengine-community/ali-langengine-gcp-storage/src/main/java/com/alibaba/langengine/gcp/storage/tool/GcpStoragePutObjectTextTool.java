...
/**
 * Google Cloud Storage Put Object Text Tool
 * Upload text content to a GCS object
 * 
 * @author LangEngine Team
 */
@Slf4j
public class GcpStoragePutObjectTextTool extends BaseTool {
    
    public GcpStoragePutObjectTextTool() {
        setName("GcpStorage.put_object_text");
        setDescription("Upload text content to a Google Cloud Storage object. " +
                "Parameters: project_id, bucket_name, object_name, content, " +
                "credentials_path (optional), content_type (optional)");
        setParameters("{\n" +
                "  \"type\": \"object\",\n" +
                "  \"properties\": {\n" +
                "    \"project_id\": {\n" +
                ...
                "    \"object_name\": {\n" +
                "      \"type\": \"string\",\n" +
                "      \"description\": \"Name of the object to create/update\"\n" +
                "    },\n" +
                "    \"content\": {\n" +
                "      \"type\": \"string\",\n" +
                ...
                "    }\n" +
                "  },\n" +
                "  \"required\": [\"project_id\", \"bucket_name\", \"object_name\", \"content\"]\n" +
                "}");
    }
    ...
    public ToolExecuteResult run(String toolInput, ExecutionContext executionContext) {
        try {
            ...
            String contentType = (String) args.getOrDefault("content_type", "text/plain");
            
            if (StringUtils.isBlank(projectId) || StringUtils.isBlank(bucketName) || 
                StringUtils.isBlank(objectName) || content == null) {
                return new ToolExecuteResult("Error: project_id, bucket_name, object_name, and content are required");
            }
            ...
            result.put("generation", blob.getGeneration());
            result.put("created", blob.getCreateTime());
            result.put("status", "success");
            
            return new ToolExecuteResult(JSON.toJSONString(result));
            
        } catch (Exception e) {
            log.error("GcpStorage.put_object_text failed", e);
