...
/**
 * Google Cloud Storage Delete Object Tool
 * Delete an object from GCS bucket
 * 
 * @author LangEngine Team
 */
@Slf4j
public class GcpStorageDeleteObjectTool extends BaseTool {
    
    public GcpStorageDeleteObjectTool() {
        setName("GcpStorage.delete_object");
        setDescription("Delete an object from a Google Cloud Storage bucket. " +
    ...
    public ToolExecuteResult run(String toolInput, ExecutionContext executionContext) {
        try {
            ...
            result.put("status", deleted ? "success" : "object not found or already deleted");
            
            return new ToolExecuteResult(JSON.toJSONString(result));
            
        } catch (Exception e) {
            log.error("GcpStorage.delete_object failed", e);
