...
public class SolrService {
    ...
    private final SolrParam param;
    private final SolrClient client;
    private final AtomicInteger cachedDimension = new AtomicInteger(-1);

    public SolrService(String serverUrl, String username, String password, String collection, SolrParam param) {
        this.collection = collection;
        this.param = param != null ? param : new SolrParam();
        this.client = new SolrClient(serverUrl, username, password, this.param);
        
        log.info("SolrService initialized for collection: {}", collection);
    }
    ...
    public void init(Embeddings embedding) {
        try {
            ...
            log.error("Failed to initialize Solr collection: {}", collection, e);
            throw new SolrException("INIT_ERROR", "Failed to initialize collection", e);
        }
    }
    ...
    private void processBatch(List<Document> documents) {
        List<Map<String, Object>> solrDocs = new ArrayList<>(documents.size());
        
        for (Document document : documents) {
            Map<String, Object> solrDoc = new HashMap<>();
            solrDoc.put(param.getFieldNameUniqueId(), document.getUniqueId());
            solrDoc.put(param.getFieldNamePageContent(), document.getPageContent());
            ...
            if (document.getEmbedding() != null) {
                ...
                solrDoc.put(param.getFieldNameEmbedding(), embeddings);
            }
            
            solrDocs.add(solrDoc);
        }
        
        client.insert(collection, solrDocs);
    }
    ...
    private int getDimension(Embeddings embedding) {
        ...
        synchronized (this) {
            ...
            if (dimension <= 0) {
                throw new SolrException("INVALID_DIMENSION", "Cannot determine valid vector dimension");
            }
    ...
    public void close() {
        if (client != null) {
            try {
                ...
                log.warn("Error occurred while closing Solr client", e);
            }
        }
    }
