...
@Slf4j
public class HuaweiSmsTool extends BaseTool {
    
    private final SmsConfiguration config;
    
    public HuaweiSmsTool() {
        this(new SmsConfiguration());
    }
    
    public HuaweiSmsTool(SmsConfiguration config) {
        this.config = config;
        setName("huawei_sms");
        setHumanName("华为云短信工具");
        setDescription("发送华为云短信");
        setParameters("{\n" +
                "  \"type\": \"object\",\n" +
                "  \"properties\": {\n" +
                "    \"phoneNumbers\": {\n" +
    ...
    public ToolExecuteResult run(String toolInput, ExecutionContext executionContext) {
        try {
            ...
            String templateId = (String) params.getOrDefault("templateId", config.getHuaweiTemplateId());
            String templateParam = (String) params.get("templateParam");
            String signName = (String) params.getOrDefault("signName", config.getHuaweiSignName());
            ...
            String errorMsg = "华为云短信发送失败：" + e.getMessage();
