...
@Slf4j
@Data
public class GoogleDriveFileManageTool extends DefaultTool {
     ...
     * 默认构造函数
     */
    public GoogleDriveFileManageTool() {
        setName("google_drive_file_manage");
        setFunctionName("manageGoogleDriveFile");
        setHumanName("Google Drive文件管理");
        setDescription("管理Google Drive文件，包括创建文件夹、删除文件、移动文件等操作");
        setParameters("{\n" +
                "  \"type\": \"object\",\n" +
                "  \"properties\": {\n" +
                "    \"action\": {\n" +
     ...
     */
    public GoogleDriveFileManageTool(GoogleDriveConfiguration configuration) {
        this();
     ...
     */
    public GoogleDriveFileManageTool(GoogleDriveClient googleDriveClient) {
        this();
    ...
    public ToolExecuteResult execute(String toolInput) {
        try {
            ...
            log.info("执行Google Drive文件管理操作，输入参数: {}", toolInput);
            ...
            log.error("Google Drive文件管理操作失败", e);
    ...
    private ToolExecuteResult createFolder(JSONObject input, JSONObject response) {
        ...
        GoogleDriveFile folder = googleDriveClient.createFolder(folderName, parentFolderId);
    ...
    private ToolExecuteResult deleteFile(JSONObject input, JSONObject response) {
        ...
        boolean success = googleDriveClient.deleteFile(fileId);
    ...
    private ToolExecuteResult moveFile(JSONObject input, JSONObject response) {
        ...
        GoogleDriveFile movedFile = googleDriveClient.moveFile(fileId, newParentId);
    ...
    private ToolExecuteResult getFileInfo(JSONObject input, JSONObject response) {
        ...
        GoogleDriveFile file = googleDriveClient.getFileInfo(fileId);
